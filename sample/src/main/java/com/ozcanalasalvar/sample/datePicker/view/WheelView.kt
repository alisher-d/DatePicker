package com.uznewmax.mytaxi.navigationFragments.ui.corporate.helper.datePicker.view
...
class WheelView : ScrollView {
    ...
    private fun init(context: Context) {
        ...
        this.addView(views)
        this.typeface = ResourcesCompat.getFont(context, R.font.lato_semibold)
    ...
    private fun createView(item: String): LinearLayout {
        ...
        tv.isClickable = true
        val textLP = LayoutParams(MATCH_PARENT, WRAP_CONTENT)
        ...
        val containerParam = LayoutParams(MATCH_PARENT, getViewMeasuredHeight(tv))
        viewContainer.layoutParams = containerParam
        viewContainer.addView(tv)
        if (0 == itemHeight || configChanged) {
            itemHeight = getViewMeasuredHeight(tv)
            views!!.layoutParams = LayoutParams(MATCH_PARENT, itemHeight * displayItemCount)
    ...
    private fun refreshItemView(y: Int) {
        ...
        for (i in 0 until childSize) {
            ...
            val item = itemView.getChildAt(0) as TextView

            if (position == i) {
                item.setTextColor(ContextCompat.getColor(context1!!, R.color.color_text))
                ...
                when (i) {
                    position - 1 -> {
                        ...
                        item.setTextColor(ContextCompat.getColor(context1!!, R.color.color_grey))
                    }
                    position - 2 -> {
                        item.setTextSize(TypedValue.COMPLEX_UNIT_SP, (textSize - 6).toFloat())
                        item.setTextColor(ContextCompat.getColor(context1!!, R.color.color_grey1))
                    }
                    else -> {
                        item.setTextSize(TypedValue.COMPLEX_UNIT_SP, (textSize - 8).toFloat())
                        item.setTextColor(ContextCompat.getColor(context1!!, R.color.color_grey2))
                    }
                }
    ...
    private fun dip2px(dpValue: Float = 1f): Int {
